...
rm $test/QUIC-Ivy/doc/examples/quic/test/test.py
cp test.py $test/QUIC-Ivy/doc/examples/quic/test/
cd $test/QUIC-Ivy/doc/examples/quic/quic_tests

printf "BUILDING TEST \n"
for j in "${tests_server[@]}"; do
    :
    ivyc target=test $j.ivy
    cp $j $test/QUIC-Ivy/doc/examples/quic/build/
    cp $j.cpp $test/QUIC-Ivy/doc/examples/quic/build/
    cp $j.h $test/QUIC-Ivy/doc/examples/quic/build/
    rm $j
    rm $j.cpp
    rm $j.h
    printf "\n"
...
mkdir $test/results/temp/

printf "Create SSLLOGFILE TEST \n"
for j in "${servers[@]}"; do
    :
    touch $test/results/temp/${j}_key.log
done
...
printf "\n"
cd $test/QUIC-Ivy/doc/examples/quic/test/
printf "TEST SERVER \n"
count=0
for j in "${tests_server[@]}"; do
    :
    ...
    for i in "${servers[@]}"; do
        ...
        until [ $k -gt $ITER ]; do
            ...
            touch $test/QUIC-Ivy/doc/examples/quic/test/temp/${count}_${i}_${j}.pcap
            chmod o=xw $test/QUIC-Ivy/doc/examples/quic/test/temp/${count}_${i}_${j}.pcap
            tshark -i lo -w $test/QUIC-Ivy/doc/examples/quic/test/temp/${count}_${i}_${j}.pcap -f "udp" &
            python test.py iters=1 server=$i test=$j > res_server.txt 2>&1
            ((k++))
            printf "\n"
            pkill tshark
            cat res_server.txt
            cp res_server.txt $test/QUIC-Ivy/doc/examples/quic/test/temp/${count}/res_server.txt
            count=$((count + 1))
        done
        cnt2=$((cnt2 + 1))
	    printf "\n"
...
cp -R $test/QUIC-Ivy/doc/examples/quic/test/temp/ $test/results
